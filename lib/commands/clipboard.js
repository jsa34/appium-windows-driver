...
commands.setClipboard = async function setClipboard (content, contentType) {
  if (contentType === 'plaintext') {
    return await exec('powershell', ['-command', `$str=[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String(${content}));Set-Clipboard -Value $str`]);
  ...
  throw new log.Error(`Could not set clipboard as ${contentType} is not a supported content type.`);
};
...
commands.getClipboard = async function getClipboard (contentType) {
  if (contentType === 'plaintext') {
    return await exec('powershell', ['-command', `$str=Get-Clipboard;[Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($str));`]);
  } else if (contentType === 'image') {
  ...
  throw new log.Error(`Could not get clipboard as ${contentType} is not a supported content type.`);
};
